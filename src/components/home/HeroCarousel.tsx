...
const HeroCarousel = () => {
    ...
    // Filter mainly meaningful auctions for hero (e.g. Live or Upcoming)
                        ...
                        <div key={slide.id} className="embla__slide flex-[0_0_100%] min-w-0 relative h-full">
                            ...
                            <div className="absolute inset-0 container flex flex-col justify-end pb-24 z-20">
                                <div className="max-w-3xl space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
                                    ...
                                    <div className="pt-4">
                                        <Button asChild size="lg" className="bg-white text-black hover:bg-white/90 text-base px-8 h-12 rounded-none">
                                            <Link to={`/auctions`}>
            ...
            <div className="absolute bottom-8 right-8 z-20 flex gap-4 items-center">
                ...
                <div className="flex gap-2 mr-4">
                    {slides.map((_, index) => (
                        <button
                            key={index}
                            className={`w-2 h-2 rounded-full transition-all ${index === emblaApi?.selectedScrollSnap() ? "bg-white w-6" : "bg-white/40 hover:bg-white/60"
                                }`}
